...
pub use sdk::{
    capabilities, known_capabilities, run as run_agent, HarnessCapabilities, RunControl, RunEvent,
    RunResult, RunStatus, RuntimeRequest, TerminationReason, SDK_PROTOCOL_VERSION,
};

/// Execute the `par` command-line application with the process arguments and
/// environment. The binary is intentionally a very small wrapper around this
/// library entrypoint so the CLI, MCP server, and SDK protocol share one core.
pub fn entrypoint() -> Result<(), String> {
    let arguments = env::args().skip(1).collect::<Vec<_>>();
    if arguments.first().map(String::as_str) == Some("sdk") {
        return sdk::run_cli(&arguments[1..]);
    }

    let action = parse_args(arguments, EnvDefaults::load())?;
...
fn run_prompt(options: cli::CliOptions) -> Result<(), String> {
    ...
    let invocation = HarnessFactory::default().build(&request)?;
...
/// Binary-friendly wrapper that prints errors consistently with earlier Parley
/// releases. Applications should use the Rust API or TypeScript SDK instead.
pub fn main() {
