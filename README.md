...
**Antigravity** — experimental. `agy --print`. The bare `agy "<prompt>"` form launches the interactive TUI (needs a `/dev/tty`) and hangs in routed/headless use, so the adapter uses `--print` for non-interactive runs. Plain `--model`. Yolo → `--dangerously-skip-permissions`.
